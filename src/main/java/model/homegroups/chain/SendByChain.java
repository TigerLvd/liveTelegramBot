...
public class SendByChain extends Chain {
    private final Pattern pattern = Pattern.compile("\\s*Ввод\\s\\s*за\\s\\s*(\\d|\\d{2})\\s\\s*(\\d|\\d{2})\\.(\\d|\\d{2})\\.(\\d{4}|\\d{2})\\s\\s*(\\d|\\d{2})\\s*", Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
